'''
Tests for profile_page app views
'''
...
class TestProfilePageViews(TestCase):
    ...
    page does not display success message if the form is not valid on
    submission.
    ...
    def test_update_profile_without_display_name(self):
        '''
        check that the view returns a status of 200.
        check that the view does not show the success message if the
        form is not valid.
        '''
        # user must be logged in to update profile
        self.client.login(
        ...
        self.assertNotIn(
            b'Your account has been updated', response.content,
            msg="Display name was left empty but success message was displayed"
        )

    def test_update_profile_with_invalid_email(self):
        '''
        Check that the view returns a status of 200.
        Check that the view does not show the success message if the form
        is not valid.
        '''
        # user must be logged in to update profile
        self.client.login(
        ...
        self.assertNotIn(
            b'Your account has been updated', response.content,
            msg="Email was invalid but success message was displayed"
        )
    ...
    def test_view_profile(self):
        ...
        response = self.client.get(
            reverse('view_profile', args=['myusername']))
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            b"blah blah blah", response.content,
            msg="About me was not displayed on profile"
            )
        self.assertIn(
            b"123456", response.content,
            msg="High score was not displayed on profile"
            )
        self.assertIn(
            b"Bucky", response.content,
            msg="Display name was not displayed on profile"
            )
        self.assertIn(
            bytes(join_date, encoding='utf-8'),
            response.content,
            msg="Join date was not displayed on profile"
            )
        self.assertIn(
            bytes(last_login, encoding='utf-8'),
            response.content,
            msg="Last login was not displayed on profile"
            )
