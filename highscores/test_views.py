''''
Highscore app tests:
- test_page_contains_usernames_and_highscores
'''
import random
...
class TestHighscoreViews(TestCase):
    ...
    def test_page_contains_usernames_and_highscores(self):
        '''
        Tests that all usernames and highscores in the database are displayed
        in the returned html.
        Checks that the context data returned is a query set of users ordered
        by high score.
        '''
        response = self.client.get(reverse('highscores'))
        self.assertEqual(response.status_code, 200)

        # Check that the correct context is sent with the correct ordering
        # from the view.
        context_data = list(response.context[0]['user_list'].all())
        ordered_user_list = list(
            User.objects.all().order_by('-profile__high_score'))
        ...
        for user in self.users:
            ...
            self.assertIn(
                bytes(str(user.profile.high_score), encoding='utf-8'),
                response.content
            )
